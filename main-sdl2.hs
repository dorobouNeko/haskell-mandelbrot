...
main = do
	...
	let
		...
		(b,color) = splitAt 4 c
 		([mw, mh],[lB, rB, uB, dB]) =
 			if null maxRes
 				then ([640,480], map strToDouble b)
 				else (map strToInt maxRes, map strToDouble b)
 		[green,blue] = if null color then [255,255] else (map strToInt color)
 		bounds = (lB,rB,uB,dB)
 		(w,h) = (rB - lB, uB - dB)
		(px,py) = M.pixelSize (mw,mh) w h
		windowSize = v2CInt (px, py)
		(s0,py') = M.points (px,py) bounds
		offset = if uB > -dB then py-py' else -(py-py')
		hue = cycle $ filter (\x -> mod x 23 == 0) ([0..255]++(reverse [1..254]))
	SDL.initializeAll
{- debug option
